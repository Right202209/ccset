...
方向键移动，`1`-`9` 选择当前窗口内对应编号的可见行，Enter 选择，Esc 返回。长列表会显示当前可见范围和总行数。表单在放弃未保存修改前会请求确认，其余情况从不询问。界面绘制在带边框的框架中：当前屏幕可用的按键显示在底部边框里，主面板的标题显示嵌套界面的完整导航路径；终端较窄时仍保留最后两级路径。终端至少 100 列宽、16 行高时，侧面板会显示当前 Agent、其配置是否存在、ccset 读写的主目录，以及最近一次的结果。框架只与其内容等高，此前的终端输出仍保留在回滚缓冲区中。如果终端无法绘制方框字符，请设置 `CCSET_ASCII=1`。
