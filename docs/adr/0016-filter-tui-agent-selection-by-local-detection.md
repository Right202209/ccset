...
an explicit empty state when none are positive. A failed detection is treated
as unavailable for that launch rather than as a reason to expose the option.
