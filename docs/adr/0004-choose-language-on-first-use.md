...
previous run; otherwise the first-run prompt; otherwise `'en'`. Presence of the
environment variable is tracked separately from its normalized value: an empty
or unknown value is an explicit English override, so it suppresses prompting
and persistence. When `CCSET_LOCALE` is set, ccset never persists it — the
variable is a per-invocation statement, and reading a durable preference out of
it would turn one scripted run into a silent permanent switch.

Argument parsing happens first. `--help` and `--version` exit without reading
settings or showing the prompt. The TTY guard runs next; a non-TTY invocation
always exits `2` as documented, without prompting or reading or writing
settings. Only a normal interactive invocation can reach the saved-locale
lookup and first-run prompt. The prompt runs regardless of `--agent`, which
skips agent selection but has no bearing on language.
...
isolated. A file that is missing, malformed, non-object, has a non-`1` version,
lacks a string locale, or names a locale ccset does not carry is treated as
*unchosen*, not as an error; the next successful choice replaces it. A failure
to persist after a successful choice is warned on stderr after the prompt render
exits and keeps the choice for this session.
...
the standing rule that every screen offers a way back. Cancellation uses the
normal user-cancel exit path and leaves no settings file behind.
...
asked nor persisted and beats a conflicting saved value; `--help`, `--version`,
and non-TTY runs never prompt; cancellation leaves no settings file; a corrupt,
schema-invalid, or unknown-locale file re-asks. Each assertion is mutation-checked per the
