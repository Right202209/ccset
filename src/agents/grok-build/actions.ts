...
async function openGlobal(ctx: Ctx): Promise<ActionResult> {
  ...
  return {
    ...
    notes: [t('grokBuild.note.configPath', { path: file.path }), t('note.preserved')],
    busyLabel: () => busy,
...
function providerForm(input: ProviderFormInput): ActionResult {
  ...
  return {
    ...
    values,
    baseline: { ...values },
    notes: [t('grokBuild.note.configPath', { path: targetPath }), t('note.preserved')],
    busyLabel: () => busy,
    submit: async (next: FormValues) =>
      runSave('grokBuild.write.providerSaved', (fresh) => saveProvider(ctx, next, fresh), busy),
  }
}
...
function providerDetail(record: ProviderRecord): string {
  if (record.baseUrl.length === 0) return t('grokBuild.status.noBaseUrl')
  return [record.baseUrl, record.modelId.length > 0 ? record.modelId : null]
  ...
  const items: ListItem[] = [
    {
      ...
      detail: t('grokBuild.action.providerAddDetail'),
...
export function grokBuildActions(): Action[] {
  return [
    {
      id: 'global',
      labelKey: 'action.global',
      detailKey: 'grokBuild.action.globalDetail',
      run: openGlobal,
    },
    {
      id: 'providers',
      labelKey: 'action.providers',
      detailKey: 'grokBuild.action.providersDetail',
      run: openProviders,
    },
