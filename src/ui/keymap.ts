...
export const KEYMAPS: Record<ScreenKind, readonly Binding[]> = {
  ...
  form: [{keys:['up','k'],labelKey:'key.moveUp'},{keys:['down','j','tab'],labelKey:'key.moveDown'},{keys:['left','right','space'],labelKey:'key.change'},{keys:['enter'],labelKey:'key.next'},{keys:['ctrl+s'],labelKey:'key.save'},{keys:['esc'],labelKey:'key.cancel'}],
...
export function pressed(input: string, key: { upArrow?: boolean; downArrow?: boolean; leftArrow?: boolean; rightArrow?: boolean; return?: boolean; escape?: boolean; ctrl?: boolean }): string[] {
  const result = input ? [key.ctrl ? `ctrl+${input}` : input] : []
